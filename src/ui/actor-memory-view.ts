...
function normalizedRatio(value: number): number { return clamp(value * 100); }
...
function previewDetails(packet: MemoryRecallPacket | null): string {
  if (!packet) return '无可用细节单元';
  if (!packet.details.length) return packet.effectiveStrength >= MEMORY_STRENGTH_LEVELS.exact ? '完整内容已作为召回文本输出' : '不附加额外细节';
...
export function renderActorMemoryTraceRow(
  state: ActorMemoryViewState,
  trace: ActorMemoryTrace,
  options: ActorMemoryViewOptions,
): string {
  const nextState: ActorMemoryViewState = {
    ...state,
    traces: [trace],
    selectedOwnerId: trace.ownerId,
    selectedTraceId: trace.id,
  };
  const selection = selectActorMemoryView(nextState);
  return renderTraceList(nextState, selection, options);
}
...
function renderMemoryGauge(trace: ActorMemoryTrace, fact: ActorMemoryFact, strength: number): string {
  ...
  const descriptions: Readonly<Record<ActorMemoryLevel, string>> = { forgotten: '只保留主体印象，强度低于 1 时不再召回', fragment: '只保留主体与行为主题，不泄露精确结果', gist: '使用有意义的语义摘要，不附加重复细节', clear: '使用有意义的语义摘要，不附加重复细节', exact: '完整输出原始事实内容' };
...
function renderOverviewTab(selection: ActorMemorySelection, options: ActorMemoryViewOptions, now: number): string {
  ...
  const packet = buildMemoryRecallPacket(trace, fact, now, `actor-memory:${trace.floor ?? 'metadata'}`, { traits: { ...DEFAULT_MEMORY_TRAITS, ...(owner.memoryTraits ?? {}) } });

...
function renderToolbar(state: SceneEventsState, visible: SceneEventVisibleRecords): string {
  ...
  return `<div class="stx-memory-scene-toolbar"><label class="stx-memory-scene-search"><span class="stx-memory-sr-only">搜索当前类别</span><ss-helper-icon name="magnifying-glass" decorative></ss-helper-icon><input ${uiControl('input')} type="search" data-scene-input="query" value="${escapeHtml(state.query)}" placeholder="${escapeHtml(searchPlaceholder(state.category))}" aria-label="搜索当前类别"></label><label class="stx-memory-scene-filter"><span class="stx-memory-sr-only">筛选当前类别</span><select ${uiControl('select')} data-scene-select="filter" aria-label="筛选当前类别">${filterOptions(state.category, state.filter)}</select></label><div class="stx-memory-scene-metrics" aria-label="当前类别统计">${metrics.map(([value, label]) => `<span><strong>${value}</strong>${escapeHtml(label)}</span>`).join('')}</div></div>`;
}
...
export function renderSceneEventsPage(state: SceneEventsState): string {
  ...
  return `<div class="stx-memory-scenes-shell"><div class="stx-memory-scene-controls">${renderScenePlanningSummary(state, owners)}${renderCategorySwitch(state)}${renderToolbar(state, visible)}</div><div class="stx-memory-scene-record-grid"><section class="stx-memory-scene-panel stx-memory-scene-list-panel" aria-label="${title}列表"><div class="stx-memory-scene-panel-head"><div><h3>${headingWithIcon(listIcon, title)}</h3></div><span>${count} 条</span></div><div class="stx-memory-scene-record-list">${list}</div></section><section class="stx-memory-scene-panel stx-memory-scene-inspector" aria-label="${title}详情">${inspector}</section><aside class="stx-memory-scene-panel stx-memory-scene-aside" aria-label="辅助信息">${renderAside(state, owners)}</aside></div></div>`;
}

export function renderSceneEventRecordRow(
  state: SceneEventsState,
  record: SceneCast | MemoryEpisode | MemoryObservation,
): string {
  const owners = ownerDirectory(state);
  if (state.category === 'event') return renderEventList(state, [record as MemoryEpisode]);
  if (state.category === 'observation') return renderObservationList(state, [record as MemoryObservation], owners);
  return renderSceneList(state, [record as SceneCast], owners);
}

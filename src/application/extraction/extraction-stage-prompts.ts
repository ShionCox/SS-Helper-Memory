...
const STAGE_RULES: Readonly<Record<ExtractionStageKey, string>> = Object.freeze({
  ...
  narrative: '顶层必须且只能包含 episodes 与 claims 两个数组；不得输出其他顶层字段。人物和地点只能引用已提供或实体阶段发放的短引用。只输出互不重复、对未来剧情确有检索价值的最小集合，episodes 最多 8 条、claims 最多 16 条；直接生成 JSON，不要先复述来源或展开分析。',
  inventory: '顶层必须且只能包含 itemCandidates 与 inventoryOperations 两个数组；不得输出其他顶层字段。旧库存只能用于比较，不能作为新数量证据。',

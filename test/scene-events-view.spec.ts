...
describe('场景与事件 v4 视图模型', () => {
  ...
  it('使用 SDK 控件契约渲染三类别、等高工具栏、三栏详情与真实字段', () => {
    ...
    expect(markup).toContain('stx-memory-scene-controls');
    ...
    expect(markup).toContain('aria-label="适应视图"');
    expect(markup).toContain('name="arrows-to-circle"');
    expect(markup).toContain('aria-label="显示范围"');
    expect(markup).toContain('name="layer-group"');
    expect(markup).toContain('aria-label="显示来源"');
    expect(markup).toContain('name="link"');
    expect(markup).toContain('aria-label="显示置信度"');
    expect(markup).toContain('name="gauge"');
    expect(markup).not.toContain('>适应视图</button>');
    expect(markup).not.toContain('>清除聚焦</button>');

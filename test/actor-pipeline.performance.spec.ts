...
describe('actor pipeline performance budget', () => {
  it('recalls 50k traces over 10k facts within the v0 300ms pipeline budget', async () => {
    ...
    const traces: ActorMemoryTrace[] = Array.from({ length: 50_000 }, (_, i) => ({ id: `t${i}`, workspaceId: 'w', chatKey: 'chat', ownerId: 'owner:actor:a', factId: `f${i % 10_000}`, sourceObservationIds: ['o'], knowledgeMode: 'experienced', privacy: 'public', strength: 80, clarity: 80, beliefConfidence: 1, emotionalSalience: 1, rehearsalCount: 0, traceRevision: 1, learnedAt: now, createdAt: now, updatedAt: now }));
    ...
    const request = { workspaceId: 'w', chatKey: 'chat', query: 'copper', now, scene: { id: 's', workspaceId: 'w', chatKey: 'chat', floor: 1, members: [], viewpointOwnerId: 'owner:actor:a', speakerOwnerIds: ['owner:actor:a'], presentOwnerIds: ['owner:actor:a'], mentionedOwnerIds: [], createdAt: now } } as const;

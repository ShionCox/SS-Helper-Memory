...
import { LLM_CAPABILITY_STATUS_V1, LLM_EMBEDDING_V1, LLM_RERANK_V1, LLM_STRUCTURED_TASK_V1, type PluginSession } from '@ss-helper/sdk';
...
describe('SDK LLM typed adapter', () => {
  it('maps structured/embed/rerank calls through public contracts with timeout and abort options', async () => {
    ...
    expect(call).toHaveBeenNthCalledWith(1, LLM_STRUCTURED_TASK_V1, expect.not.objectContaining({ timeoutMs: expect.anything() }), { signal });
  ...
  it('reads the real LLM capability service before reporting a recall route as available', async () => {
    ...
    expect(call).toHaveBeenCalledWith(LLM_CAPABILITY_STATUS_V1, {

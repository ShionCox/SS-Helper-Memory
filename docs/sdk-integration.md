...
Memory 通过 v0 服务契约调用提取、embedding、rerank 与单次结构化 Capture。设置 schema 与工作台 popup 由 Core 承载，具体 UI 和所有记忆操作仍由 Memory 实现。
...
卡内多角色模型使用 `actors`、`actor-aliases`、`episodes`、`observations`、`facts`、`evidence`、`fact-heads`、`memory-traces`、`scene-casts`、`profiles`、`dream-jobs` 和审计集合；冲突规则、知识投影、清理规则与召回策略都不属于 SDK 公共实现。宿主角色卡/群组只决定 workspace，不生成 MemoryOwner。
